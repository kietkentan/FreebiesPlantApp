...
fun checkAppStart(context: Context, sharedPreferences: SharedPreferences): AppStart? {
    ...
    try {
        ...
        sharedPreferences.edit()
            ...
            .apply() // must use commit here or app may not update prefs in time and app will loop into walkthrough
...
fun checkAppStart(currentVersionCode: Int, lastVersionCode: Int): AppStart {
    return if (lastVersionCode == -1) {
        ...
        Log.w("TAG_U", "Current version code ($currentVersionCode) is less then the one recognized on last startup ($lastVersionCode). Defensively assuming normal app start.")
        AppStart.NORMAL
    } else {
        AppStart.NORMAL
    }
